...
if __name__ == "__main__":
    ...
    classifier = AdaBoostClassifier(sklearn.tree.DecisionTreeClassifier,1)
    classifier.fit(X_train, y_train)
    print(np.mean(classifier.predict(X_test)==np.transpose(y_test)))

   # X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=42)
    classifier = AdaBoostClassifier(sklearn.tree.DecisionTreeClassifier, 2)
    classifier.fit(X_train, y_train)
    print(np.mean(classifier.predict(X_test) == np.transpose(y_test)))

    #X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=42)
    classifier = AdaBoostClassifier(sklearn.tree.DecisionTreeClassifier, 3)
    classifier.fit(X_train, y_train)
    print(np.mean(classifier.predict(X_test) == np.transpose(y_test)))
    ...
    #X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=42)
    classifier = AdaBoostClassifier(sklearn.tree.DecisionTreeClassifier, 5)
    classifier.fit(X_train, y_train)
    print(np.mean(classifier.predict(X_test) == np.transpose(y_test)))
...
>>>>>>> 192ec4a0c890bc74b734a465dca1d7c57d044772
=======
from PIL import Image
import feature
import numpy as np
